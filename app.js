...
// helmet to secure HTTP headers (adds security headers)
...
// CSRF PROTECTION
// ============================================
// Cross-Site Request Forgery (CSRF) protection using csrf-csrf (Double Submit Cookie Pattern).
// Every state-changing POST must include a matching token in the form body (_csrf) or
// request header (x-csrf-token). The server validates the token against the cookie.
...
// Embed hidden _csrf field in HTML forms served to the browser
function getCsrfField(req, res) {
  return `<input type="hidden" name="_csrf" value="${generateCsrfToken(req, res)}">`;
}
...
// csrf-csrf validates the _csrf token before this handler runs
app.post('/register', async (req, res) => {
  ...
  if (!/^[a-zA-Z0-9_-]{3,30}$/.test(username)) {
    return res.status(400).send('Username must be 3-30 characters (letters, numbers, underscores, hyphens)');
  }
  ...
  // VULNERABILITY 5 Fixed: Sanitize username, email, and bio to prevent XSS
...
app.post('/login', loginLimiter, async (req, res) => {
  ...
  const user = users.find(u => u.username === username || u.username === validator.escape(username));
  if (!user) {
...
app.post('/profile/edit', (req, res) => {
  ...
  req.user.email = validator.escape(email);
...
<body>
  ...
  <div class="container" style="margin-top: 80px;">
    <div class="card">
      <div class="message error">New password must be at least 8 characters</div>
      <div class="links">
        <a href="/password/change" class="btn btn-primary">Try Again</a>
        <a href="/profile?user=${req.user.username}" class="btn btn-secondary">Back to Profile</a>
      </div>
...
<head>
  ...
  <link rel="stylesheet" href="/css/style.css">
  <meta name="csrf-token" content="${generateCsrfToken(req, res)}">
</head>
<body>
  ${getNavbar(req.user)}
...
<body>
  ...
  <div class="admin-container" style="margin-top: 80px;">
    <div class="container">
      <div class="card">
        <div class="message error">Username must be 3-30 characters (letters, numbers, underscores, hyphens)</div>
        <a href="/admin/users/${req.params.username}/edit" class="btn btn-primary">Try Again</a>
      </div>
    </div>
  </div>
</body>
...
<body>
  ...
  user.email = validator.escape(email);
  if (password) {
...
<body>
  ...
  <div class="admin-container" style="margin-top: 80px;">
    <div class="container">
      <div class="card">
        <div class="message error">Password must be at least 8 characters</div>
        <a href="/admin/users/add" class="btn btn-primary">Try Again</a>
      </div>
    </div>
  </div>
  ...
  if (users.some(u => u.username === username || u.username === validator.escape(username))) {
    return res.send(`<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
app.get('/admin/export', (req, res) => {
  ...
  const exportUsers = users.map(u => ({ ...u, password: undefined }));
  if (format === 'csv') {
    const csv = 'username,email,bio,createdAt,lastLogin,isAdmin\n' +
      exportUsers.map(u => `"${u.username}","${u.email}","${u.bio || ''}","${u.createdAt}","${u.lastLogin || ''}","${u.isAdmin}"`).join('\n');
  ...
  res.send(JSON.stringify(exportUsers, null, 2));
});

// IMPORT USERS
// AJAX import sends x-csrf-token header (see admin dashboard importUsers script)
    ...
    try {
      ...
      for (const u of importedUsers) {
        ...
        if (u.password) {
          ...
          u.password = await bcrypt.hash('imported123', 10);
        }
...
// Reject forged requests when CSRF token is missing or invalid
